...
# segemetation based on a threshold to enable the teseract OCR engine to work
def img2bw(img,th):
    h,w = img.shape[0:2]
    img2 = img
    ...
    for box in boxes:
        ...
        if label >= 0:
            ...
            cv2.rectangle(img=image, pt1=(box.xmin, box.ymin), pt2=(box.xmax, box.ymax), color=get_color(label),
                          thickness=5)
            with open('dim.npy', 'wb') as f:
                np.save(f,[box.ymin+4,box.ymax+4, box.xmin+4,box.xmax+4])
